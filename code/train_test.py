...
from my_model.Model import DeliModel
...
class Train:
    ...
    def __init__(self, args):
        ...
        try:
            ...
            self.val_data = get_val_data()
            self.test_data = get_test_data()
            print("get data")

            # 模型建立
            self.model = DeliModel(config)
    ...
    def run_epoch(self, sess, input_data, net_state=1):
        """
        ...
        try:
            for batch_data in input_data:
                ...
                feed_dict = self.model.create_feed_dict(batch_data=batch_data, net_state=net_state, train=self.config.istrain, dropout_keep_prob=self.config.dropout_keep_prob)
                (accuarcy, global_step, summary, opt_loss, ce_loss, lr, _) = sess.run(
                    [self.model.accuracy, self.model.global_step, self.merged,
                     self.model.opt_loss, self.model.loss, self.model.learning_rate, self.model.train_op],
                    feed_dict = feed_dict
                )
                ...
                print('step: %d, time: %.3fs, loss: %.4f, opt_loss: %.4f, accu: %.4f, lr: %f' %(
                    step, time_stamp, ce_loss, opt_loss, accuarcy, lr
                ))
        ...
        print('==' * 20)
        print('epoch: %d, total loss: %.4f, total ce_loss: %.4f, total timestamp: %.3fs, total acc: %.4f' % (
            epoch, np.mean(total_loss), np.mean(total_ce_loss), np.mean(collect_time), np.mean(accuarcy_collect)
        ))

        return np.mean(total_loss), np.mean(total_ce_loss), np.mean(accuarcy_collect)


    def fit_predict(self, sess, input_data, net_state=1):
        ...
        try:
            for batch_data in input_data:
                step += 1
                total_labels = np.concatenate([total_labels, batch_data[2]])
                feed_dict = self.model.create_feed_dict(batch_data=batch_data, net_state=net_state, train=self.config.istrain, dropout_keep_prob=1.0)

                start_stamp = time.time()
                global_step, summary, ce_loss, opt_loss, y_pred = sess.run(
                    [self.model.global_step, self.merged, self.model.loss, self.model.opt_loss, self.model.y_pred],
                    feed_dict=feed_dict
                )
        ...
        return np.mean(total_ce_loss), np.mean(total_loss), total_labels, total_pred, accuracy

    def test_case(self, sess, input_data, net_state=1, onset='VALIDATION'):
        print('#' * 20, 'ON ' + onset + ' SET START ', '#' * 20)
        print('='*10 + ' '.join(sys.argv) + '=' * 10)
        epoch = self.get_epoch(sess)
        ce_loss, opt_loss, label, pred, accuracy = self.fit_predict(sess, input_data, net_state)

        precision, recall, f1 = score(pred, label)
        print('%d the Epoch ---- %s Overall accuary is: %f, Overall ce_loss is: %f, opt_loss is: %f' %(epoch, onset, accuracy, ce_loss, opt_loss))
        print('%d the Epoch ---- %s Overall precision is: %f, Overall recall is: %f, Overall f1 is: %f' % (epoch, onset, precision, recall, f1))
        logging.info('%d the Epoch ---- %s Overall accuary is: %f, Overall ce_loss is: %f, opt_loss is: %f' %(epoch, onset, accuracy, ce_loss, opt_loss))
    ...
    def train_run(self):
        ...
        with tf.Session(config=conf) as sess:
            ...
            # 第一次网络训练
            self.config.net_state = 1
            for _ in range(self.config.max_first_epochs):

                epoch = self.get_epoch(sess)
                print("=" * 20 + "Epoch ", epoch, "=" * 20)
                ce_loss, opt_loss, accuracy  = self.run_epoch(sess, self.train_data, self.config.net_state)
                ...
                val_accuracy, val_loss = self.test_case(sess, self.val_data, net_state=self.config.net_state, onset='VALIDATION')
                test_accuracy, test_loss = self.test_case(sess, self.test_data, net_state=self.config.net_state, onset='TEST')
                ...
                if best_accuracy_1 < val_accuracy:
            ...
            # 第二次网络训练
            self.config.net_state = 2
            update_global_step = tf.assign(self.model.global_step, 0)
            update_epoch = tf.assign(self.model.on_epoch, 0)
            new_global_step = sess.run(update_global_step)
            new_update_epoch = sess.run(update_epoch)

            best_loss_2 = np.Inf
            best_accuracy_2 = 0
            best_val_epoch_2 = self.get_epoch(sess)

            print("new global step is %d, new_update_epoch is %d" % (new_global_step, new_update_epoch))
            for _ in range(self.config.max_second_epochs):
                ...
                self.save_loss_accu(self.args.weight_path + '/summary.log', train_loss=ce_loss,
                                    valid_loss=val_loss, test_loss=test_loss,
                                    valid_accu=val_accuracy, test_accu=test_accuracy, epoch=epoch)
                if best_accuracy_2 < val_accuracy:
                    best_accuracy_2 = val_accuracy
                    best_val_epoch_2 = epoch
                    if not os.path.exists(self.args.weight_path):
                        os.makedirs(self.args.weight_path)
                    logging.info('best epoch is %dth epoch' % best_val_epoch_2)
