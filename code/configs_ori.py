...
from my_utils.time_counter import TimeCounter

# class Configs(object):
#     def __init__(self):
#         self.project_dir = os.getcwd() # 获取当前路径
#         self.dataset_dir = join(self.project_dir, 'dataset')
#
#         parser = argparse.ArgumentParser()
#         parser.register('type', 'bool', (lambda x: x.lower() in ('True', "yes", "true", "t", "l")))
#
#         # @ ----- control -----
...
#                             help='specify which per-trained model to be load')
#
#         # @ ------ training -------
#         parser.add_argument('--max_epoch', type=int, default=20, help='max epoch number')
#         parser.add_argument('--num_steps', type=int, default=1000, help='max steps num')
...
#         parser.add_argument('--dropout', type=float, default=0.6, help='dropout keep prob')
...
#
#         self.processed_name = 'processed' + self.get_params_str(['dataset_type', 'lower_word', 'use_glove_unk_token', 'glove_corpus', 'word_embedding_length']) + '.pickle'
...
#             self.model_nam = self.get_params_str(params_name_list)
#         else:
...
#         self.dict_path = join(self.ckpt_dir, self.model_ckpt_name)
#
#         # dtype
#         self.floatX = 'float32'
#         self.intX = 'int32'
