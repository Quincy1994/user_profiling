...
from Config import *
from model.han_model import HAN
from Metrics import score
from data_iterator import get_han_data
    ...
    def run_epoch(self, sess, input_data):
        ...
        try:
            for batch_data in input_data:
                batch_x, batch_sNum, batch_wNum, batch_labels = batch_data[0], batch_data[1], batch_data[2], batch_data[3]
                step += 1
    ...
    def train_run(self):
        ...
        with tf.Session(config=conf) as sess:
            ...
            for _ in range(config.max_epochs):
                train_data = get_han_data(config.train_data_path, shuffle=True)
                valid_data = get_han_data(config.val_data_path, shuffle=False)
                test_data = get_han_data(config.test_data_path, shuffle=False)
                print("get data")
                epoch = self.get_epoch(sess)
